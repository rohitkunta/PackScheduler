...
public class StudentDirectoryTest {
	...
	/**
	 * Tests StudentDirectory.loadStudentsFromFile() when the file does not exist.
	 */
	@Test
	public void testInvalidLoadStudentsFromList() {
		StudentDirectory sd = new StudentDirectory();

		// Test invalid file
		Exception exception = assertThrows(IllegalArgumentException.class,
				() -> sd.loadStudentsFromFile("test-files/nonexistent.txt"));
		assertEquals("Unable to read file test-files/nonexistent.txt", exception.getMessage(),
	...
	/**
	 * Tests that student cannot be added if the password is null
	 */
	@Test
	public void testAddStudentNullPassword() {
		StudentDirectory sd = new StudentDirectory();

		// Test null password
		Exception exception = assertThrows(IllegalArgumentException.class,
				() -> sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, null, PASSWORD, MAX_CREDITS));
		assertEquals("Invalid password", exception.getMessage(), "Incorrect exception thrown with null password");
	}

	/**
	 * Tests that student cannot be added that already exists in directory
	 */
	@Test
	...
	/**
	 * Tests the the getStudentById method that takes in an id string and returns the student attached to it
	...
	public void testGetStudentById() {
		...
		assertEquals(LAST_NAME, sd.getStudentById(ID).getLastName());
		assertEquals(ID, sd.getStudentById(ID).getId());
		assertEquals(EMAIL, sd.getStudentById(ID).getEmail());
		assertEquals(MAX_CREDITS, sd.getStudentById(ID).getMaxCredits());
		assertEquals(null, sd.getStudentById("abcdefg"));
