...
//		assertEquals(0, c.getCourseCatalog().length);
	...
	public void testGetStudentDirectory() {
		...
		sd.addStudent("john", "smith", "johnsmith", "JS@gm.com", "boom", "boom", 0);
		assertEquals(1, sd.getStudentDirectory().length);

	}

	/**
	 * tests user logging out
	 */
	@Test
	public void testLoginAndLogout() {
		manager.getStudentDirectory().addStudent("john", "smith", "johnsmith", "JS@gm.com", "boom", "boom", 0);
		manager.login("johnsmith", "boom");
		manager.logout();
		assertNull(manager.getCurrentUser());
		
		
		Exception e = assertThrows(IllegalArgumentException.class, () -> manager.login("2", "2"));
	...
	public void testGetCurrentUser() {
		manager.getStudentDirectory().addStudent("john", "smith", "johnsmith", "JS@gm.com", "boom", "boom", 15);
		assertEquals(1, manager.getStudentDirectory().getStudentDirectory().length);
		assertTrue(manager.login("johnsmith", "boom"));
		User u = manager.getCurrentUser();
		assertEquals("john", u.getFirstName());
		assertEquals("smith", u.getLastName());
		assertEquals("johnsmith", u.getId());
		assertEquals("JS@gm.com", u.getEmail());
	...
	void testHashPW() {
		...
		Exception e = assertThrows(NoSuchAlgorithmException.class, () -> MessageDigest.getInstance(HASH_ALGORITHM));
		assertEquals(" MessageDigest not available", e.getMessage());
